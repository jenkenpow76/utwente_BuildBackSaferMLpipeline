...
==================
Stage 4 | EVI — Stepwise OLS Regression

Input  : analysis_dataset.csv  (from Stage 05b)
Outputs: evi_stepwise_stage1_results.csv   — domain stepwise results
         evi_stepwise_stage2_results.csv   — Individual Model (IM)
         evi_stepwise_stage3_results.csv   — Composite Model (CM)
         evi_stepwise_diagnostics.png      — residual diagnostic plots
         evi_stepwise_coefficients.png     — coefficient comparison plot

Description
-----------
Mirrors stage08_fvi_stepwise.py exactly. Four-stage stepwise OLS regression:
  Stage 1: Domain-level stepwise (PIN=0.05, POUT=0.10) across 5 domains.
           Domains mirror FVI exactly; Flood_Experience replaced by
           Earthquake_Experience.
  Stage 2: Joint Individual Model (IM) from Stage 1 significant variables.
  Stage 3: Composite Model (CM) using MAO net scores and EQ composite.
  Stage 4: Diagnostics — VIF, Durbin-Watson, residual plots.

DV: EVI_norm_1_5 (1–5 scale, matches FVI_norm_1_5).
...
"""
Stepwise Linear Regression: EVI_norm_1_5 as Dependent Variable
===============================================================
...
Outputs:
  evi_stepwise_stage1_results.csv   — domain-level stepwise results
  evi_stepwise_stage2_results.csv   — joint Individual Model results
  evi_stepwise_stage3_results.csv   — Composite Model results
  evi_stepwise_diagnostics.png      — residual plots
  evi_stepwise_coefficients.png     — coefficient plot for both models
"""
...
# This stage PRODUCES evi_im_predictors.csv, so we use producer_evi mode
# which skips the EVI_IM_PREDICTORS check (any stale file is about to be
# replaced by this stage's output).
assert_no_drift("producer_evi")
...
df["EVI_norm_1_5"] = pd.to_numeric(df["EVI_norm_1_5"], errors="coerce")
...
# ── Updated from evi_bivariate_screening_significant.csv (32 variables, p<0.05) ──
# ── Updated from evi_bivariate_screening_significant.csv ─────────────────
# Candidate columns are the full pool that passed bivariate screening (p<0.05).
# The domains and variables mirror FVI_4 exactly, except:
#   - Flood Experience replaced by Earthquake Experience
#   - MAO raw scores (0-100) used throughout
candidate_cols = EVI_ALL_DOMAIN_VARS
...
# Domain groups for Stage 1 stepwise — full candidate pools per domain.
# Variables advancing from bivariate screening (p<0.05) are listed.
# The stepwise procedure within each domain selects the final subset.
# Domains mirror FVI_4 exactly; Flood_Experience is replaced by
# Earthquake_Experience with EVI-specific variables.
DOMAINS = EVI_DOMAINS
...
print("=" * 60)
print("STAGE 1: Domain-level stepwise regressions (EVI)")
print("=" * 60)
...
stage1_df.to_csv("evi_stepwise_stage1_results.csv", index=False)
...
if joint_pool:
    ...
    if sel_joint and fit_joint:
        ...
        pd.DataFrame(s2_rows).to_csv(
            "evi_stepwise_stage2_results.csv", index=False)
        print("\nStage 2 results saved.")

        # ── Write retained predictor list for Stage 16 ────────────────────
        # Stage 16 (mixed-effects) reads this file instead of the hardcoded
        # EVI_IM_PREDICTORS list in pipeline_config.py. This ensures the
        ...
        pd.DataFrame({"predictor": sel_final}).to_csv(
            "evi_im_predictors.csv", index=False)
        print("Predictor list saved: evi_im_predictors.csv "
              f"({len(sel_final)} variables → Stage 16)")
...
# Earthquake experience composite: sum of EQ experience variables
df["EQ_Exp_composite"] = (
    pd.to_numeric(df["AW01_my_house_survived_the_last_ear"], errors="coerce").fillna(0) +
    pd.to_numeric(df["DE01_earthquake"],                     errors="coerce").fillna(0) +
    pd.to_numeric(df["DE02_earthquake"],                     errors="coerce").fillna(0)
)
...
if sel_cm and fit_cm:
    ...
    pd.DataFrame(s3_rows).to_csv(
        "evi_stepwise_stage3_results.csv", index=False)
...
def make_diagnostic_plots(fit, title, axes):
    ...
    axes[0].axhline(-3, color="#D94F3D", linewidth=0.8, linestyle="--")
    axes[0].set_xlabel("Predicted EVI", fontsize=9)
...
fig.suptitle("Regression Diagnostics: EVI Stepwise Models\n"
...
plt.tight_layout()
plt.savefig("evi_stepwise_diagnostics.png",
