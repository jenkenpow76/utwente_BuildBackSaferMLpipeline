...
=======================
Stage 6 | Mixed-Effects (Multilevel) Model

Input  : analysis_dataset.csv  (from Stage 05b)
Outputs: fvi_mixed_effects_results.csv    — fixed effects table
         fvi_mixed_effects_variance.csv   — variance components and ICC
         fvi_mixed_effects_comparison.csv — OLS vs mixed-effects comparison
         fvi_mixed_effects_plot.png       — coefficient comparison plot
         fvi_random_intercepts.png        — caterpillar plot of random intercepts

Description
-----------
Fits a random-intercept multilevel model with households nested within
communities (VDC/ward). The Intra-class Correlation (ICC) quantifies how
much FVI variance is attributable to community-level clustering. Results are
compared against the Stage 4 OLS Individual Model to assess robustness.

Model: FVI_ij = (β0 + u0j) + Σβk*Xk_ij + e_ij

This is the FINAL VALIDATED FVI score. The column FVI_norm_1_5 in
imputed_FVI_scores.csv is the recommended input for composite index
construction (FVI + EVI).
...
Updated to align with 4_2__fvi_stepwise_updated.ipynb:
  - FILE_PATH: imputed_FVI_scores.csv (contains FVI_norm_1_5 + all predictors)
  - PREDICTORS: 10-variable Individual Model from updated Stage 2
  - Coverage filter removed: imputed_FVI_scores.csv already filters by
...
Outputs:
  fvi_mixed_effects_results.csv     — fixed effects table
  fvi_mixed_effects_variance.csv    — variance components and ICC
  fvi_mixed_effects_comparison.csv  — OLS vs mixed-effects comparison
  fvi_mixed_effects_plot.png        — coefficient comparison plot
  fvi_random_intercepts.png         — caterpillar plot of random intercepts
...
# This stage is an FVI-side consumer (reads fvi_im_predictors.csv).
# We use consumer_fvi mode which verifies the FVI invariants but skips
# the EVI side, because Stage 18 (the EVI producer) runs later in the
# pipeline and the EVI IM_PREDICTORS file may not yet reflect the
# current candidate pool at this point.
assert_no_drift("consumer_fvi")

# ── Load Individual Model predictor list from Stage 08 output ────────────────
# Stage 08 writes fvi_im_predictors.csv after its stepwise selection completes.
...
# Falls back to FVI_IM_PREDICTORS from pipeline_config.py if the file does
# not exist (e.g. when running Stage 10 standalone).
_im_csv = Path.cwd() / "fvi_im_predictors.csv"
if _im_csv.exists():
    import pandas as _pd_im
    _LOADED_PREDICTORS = _pd_im.read_csv(_im_csv)["predictor"].tolist()
    print(f"Loaded {len(_LOADED_PREDICTORS)} predictors from fvi_im_predictors.csv")
else:
    _LOADED_PREDICTORS = FVI_IM_PREDICTORS
    print("fvi_im_predictors.csv not found — using FVI_IM_PREDICTORS from config")
...
# =============================================================================
# imputed_FVI_scores.csv is the scored dataset produced by fvi_cimden.py.
# It contains FVI_norm_1_5, all MAO/socioeconomic predictors, GPS, and
# community identifiers. The raw survey file does NOT contain FVI_norm_1_5.
...
# Individual Model predictors — loaded from Stage 08 output (fvi_im_predictors.csv).
# Falls back to FVI_IM_PREDICTORS from pipeline_config.py if run standalone.
...
fe_df.to_csv("fvi_mixed_effects_results.csv", index=False)
...
vc_df.to_csv("fvi_mixed_effects_variance.csv", index=False)
print("\nResults saved: fvi_mixed_effects_results.csv, fvi_mixed_effects_variance.csv")
...
comp_df.to_csv("fvi_mixed_effects_comparison.csv", index=False)
print("Saved: fvi_mixed_effects_comparison.csv")
...
ax1.set_xlabel(
    "β coefficient  (positive = higher FVI = more vulnerable)",
    fontsize=10,
)
ax1.set_title(
    f"Fixed Effect Coefficients — FVI: OLS vs Mixed-Effects Model\n"
...
plt.tight_layout()
plt.savefig("fvi_mixed_effects_plot.png", dpi=200, bbox_inches="tight")
plt.close()
print("\nFigure 1 saved: fvi_mixed_effects_plot.png")
...
ax2.set_title(
    "FVI — Caterpillar Plot: Community-level Random Intercepts\n"
...
plt.tight_layout()
plt.savefig("fvi_random_intercepts.png", dpi=200, bbox_inches="tight")
plt.close()
print("Figure 2 saved: fvi_random_intercepts.png")
...
ri_df.to_csv("fvi_random_intercepts.csv", index=False)
print("Saved: fvi_random_intercepts.csv")
