...
==================
Stage 5 | Spatial Autocorrelation (Moran's I)

Input  : imputed_FVI_scores.csv  (from Stage 0c)
Outputs: fvi_morans_sensitivity.csv       — Moran's I across distance thresholds
         fvi_morans_correlogram.png       — Moran's I correlogram
         fvi_residual_map.png             — GPS map of regression residuals
         fvi_residual_by_community.png    — boxplot by VDC/ward
         fvi_local_morans_results.csv     — LISA per-household statistics
         fvi_local_morans_map.png         — LISA cluster map

Description
-----------
Tests whether residuals from the Individual Model (Stage 4) exhibit
spatial autocorrelation using global Moran's I across distance thresholds
and Local Indicators of Spatial Association (LISA). Results inform whether
the OLS independence assumption holds or whether multilevel modelling is
warranted.
...
# FILE_PATH = str(Path.cwd() / "imputed_FVI_scores.csv") resolves to OUTPUT_DIR/imputed_FVI_scores.csv.
# ─────────────────────────────────────────────────────────────────────────────

"""
Autocorrelation Diagnosis: Spatial and Community-level Clustering
=================================================================
Tests whether residuals from the FVI stepwise regression are 
...
Outputs:
  fvi_gps_coverage.txt          - GPS coverage by VDC
  fvi_morans_correlogram.png    - Moran's I across distance thresholds
  fvi_morans_sensitivity.csv    - Moran's I table (all thresholds)
  fvi_morans_i_results.txt      - Moran's I final report
  fvi_residual_map.png          - GPS map of residuals
  fvi_residual_by_community.png - boxplot by VDC/ward
"""
...
FILE_PATH = str(Path.cwd() / "imputed_FVI_scores.csv")   # scored dataset: contains FVI_norm_1_5,
...
# Stage 13 writes fvi_im_predictors.csv after stepwise selection completes.
...
# Falls back to FVI_IM_PREDICTORS from pipeline_config if the file is absent.
_im_csv_fvi = Path.cwd() / "fvi_im_predictors.csv"
if _im_csv_fvi.exists():
    import pandas as _pd_im
    _LOADED_IM_PREDICTORS = _pd_im.read_csv(_im_csv_fvi)["predictor"].tolist()
    print(f"Loaded {len(_LOADED_IM_PREDICTORS)} predictors from fvi_im_predictors.csv")
else:
    _LOADED_IM_PREDICTORS = FVI_IM_PREDICTORS
    print("fvi_im_predictors.csv not found — using FVI_IM_PREDICTORS from pipeline_config")
...
with open("fvi_gps_coverage.txt", "w") as f:
...
print("\nGPS coverage report saved: fvi_gps_coverage.txt")
...
sensitivity_df.to_csv("fvi_morans_sensitivity.csv", index=False)
print("\nSensitivity results saved: fvi_morans_sensitivity.csv")
...
with open("fvi_morans_i_results.txt", "w") as f:
...
ax0.set_title(
    "Spatial Correlogram — Moran's I at Multiple Distance Thresholds (FVI)\n"
...
plt.tight_layout()
plt.savefig("fvi_morans_correlogram.png", dpi=200, bbox_inches="tight")
plt.close()
print("\nFigure 1 saved: fvi_morans_correlogram.png")
...
ax1.set_title(
    "Spatial Distribution of OLS Residuals — FVI Individual Model (IM)\n"
...
plt.tight_layout()
plt.savefig("fvi_residual_map.png", dpi=200, bbox_inches="tight")
plt.close()
print("\nFigure 2 saved: fvi_residual_map.png")
...
ax2.set_ylabel("OLS Residual (FVI scale 1-5)", fontsize=10)
ax2.set_title(
    "FVI — OLS Residuals by Community (VDC/Ward), Individual Model (IM)\n"
...
plt.savefig("fvi_residual_by_community.png", dpi=200, bbox_inches="tight")
plt.close()
print("Figure 3 saved: fvi_residual_by_community.png")
print("\nDiagnosis complete.")
...
# Set axes extent BEFORE fetching OSM tiles so the tile fetcher knows
# which tiles to download. A 4% margin prevents points sitting on the edge.
...
# CartoDB Voyager: rivers visible in pale blue — relevant for flood maps.
...
#   here to FVI regression residuals.
#   https://doi.org/10.1007/s13753-018-0192-7
#
# Outputs:
#   fvi_local_morans_results.csv   — per-household I_i, z_i, p_i, cluster type
#   fvi_local_morans_map.png       — LISA cluster map
#   fvi_local_morans_summary.txt   — cluster frequency table
...
lisa_df.to_csv("fvi_local_morans_results.csv", index=False)
print(f"\nResults saved: fvi_local_morans_results.csv ({len(lisa_df)} observations)")
...
with open("fvi_local_morans_summary.txt", "w") as f:
...
print("Summary saved: fvi_local_morans_summary.txt")
...
# CartoDB Voyager: rivers visible — contextually important for flood LISA map.
_osm_add_basemap(
    ax4,
    zoom="auto",
    provider="carto_voyager",
...
ax4.set_title(
    "LISA Cluster Map — Local Moran's I of OLS Residuals (FVI)\n"
...
plt.tight_layout()
plt.savefig("fvi_local_morans_map.png", dpi=200, bbox_inches="tight")
plt.close()
print("Figure 4 saved: fvi_local_morans_map.png")
print("\nDiagnosis complete.")
