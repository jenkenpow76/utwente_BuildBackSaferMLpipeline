"""
stage06_fvi_descriptives.py
======================
Stage 2 | Descriptives and Frequencies

Input  : imputed_FVI_scores.csv  (written by Stage 0c)
Outputs: fvi_distribution.png         — histogram of FVI_norm_1_5
         fvi_indicator_frequencies.png — stacked bar chart per indicator

Description
-----------
Visualises the distribution of the composite FVI score and the
frequency of vulnerability levels (Low/Medium/High) for each of the
11 CIMDEN indicators. Prints descriptive statistics to console.
...
# INPUT_CSV = str(Path.cwd() / "imputed_FVI_scores.csv") resolves to OUTPUT_DIR/imputed_FVI_scores.csv.
# ─────────────────────────────────────────────────────────────────────────────

"""
FVI CIMDEN — Descriptives and Frequencies
==========================================

Input  : imputed_FVI_scores.csv   (output of fvi_cimden.py)
Outputs: fvi_distribution.png
         fvi_indicator_frequencies.png
...
# ── Paths ─────────────────────────────────────────────────────────────────────
INPUT_CSV = str(Path.cwd() / "imputed_FVI_scores.csv")   # update path if running from a different directory
OUT_HIST  = str(Path.cwd() / "fvi_distribution.png")
OUT_BARS  = str(Path.cwd() / "fvi_indicator_frequencies.png")
...
fvi_numeric_cols = [
    "FVI_norm_1_5", "FVI_CLASS",
    "FVI1_GeoPos_LVL",   "FVI2_RoofMat_LVL",  "FVI3_RoofConn_LVL",
    "FVI4_WallMat_LVL",  "FVI5_Elev_LVL",     "FVI6_Geom_LVL",
    "FVI7_Overhang_LVL", "FVI8_Apron_LVL",    "FVI9_Drain_LVL",
    "FVI10_Floors_LVL",  "FVI11_LWratio_LVL",
]
...
ax.set_xlim(1.0, 5.0)
ax.set_xlabel("Composite FVI (1–5 scale)", fontsize=13, labelpad=8)
ax.set_ylabel("Number of houses",          fontsize=13, labelpad=8)
ax.set_title(
    f"Distribution of Composite Flood Vulnerability Index\n"
...
indicators = [
    # ("FVI1_GeoPos_LVL",   "FVI1  Geographical position"),
    ("FVI2_RoofMat_LVL",  "FVI2  Roof material"),
    ("FVI3_RoofConn_LVL", "FVI3  Roof connections"),
    ("FVI4_WallMat_LVL",  "FVI4  Wall material"),
    ("FVI5_Elev_LVL",     "FVI5  Stilts / elevation"),
    ("FVI6_Geom_LVL",     "FVI6  Building geometry"),
    ("FVI7_Overhang_LVL", "FVI7  Roof overhang"),
    ("FVI8_Apron_LVL",    "FVI8  Apron presence"),
    ("FVI9_Drain_LVL",    "FVI9  Site drainage"),
    ("FVI10_Floors_LVL",  "FVI10 Number of storeys"),
    ("FVI11_LWratio_LVL", "FVI11 Length-to-width ratio"),
]
...
ax2.set_title(
    "Flood Vulnerability Indicator Score Frequencies\n"
...
print("\n" + "=" * 58)
print("COMPOSITE FVI — DESCRIPTIVE STATISTICS  (1–5 scale)")
print("=" * 58)
for lbl, val in [
    ("n",      f"{n_total:,}"),
    ("Mean",   f"{mean_v:.3f}"),
...
print("\n" + "=" * 58)
print("FVI CLASS FREQUENCIES")
print("=" * 58)
class_map   = {1: "Low", 2: "Medium", 3: "High"}
class_freqs = df["FVI_CLASS"].value_counts().sort_index()
